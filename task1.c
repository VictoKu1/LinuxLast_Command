...
static struct last_timefmt timefmts[] = {
	...
	[LAST_TIMEFTM_CTIME] = {.name = "full", .in_len = 24, .out_len = 26, .in_fmt = LAST_TIMEFTM_CTIME, .out_fmt = LAST_TIMEFTM_CTIME},
	[LAST_TIMEFTM_ISO8601] = {.name = "iso", .in_len = 25, .out_len = 27, .in_fmt = LAST_TIMEFTM_ISO8601, .out_fmt = LAST_TIMEFTM_ISO8601}};
...
static void __attribute__((__noreturn__)) usage(const struct last_control *ctl)
{
	...
	fprintf(out, _(" %s [options] [<username>...] [<tty>...]\n"), program_invocation_short_name);
							  ...
							  const char *filename)
{
	...
	while (!quit)
	{
		...
		case NEW_TIME:
			...
			{
				strcpy(ut.ut_line,
					   ut.ut_type == NEW_TIME ? "new time" : "old time");
